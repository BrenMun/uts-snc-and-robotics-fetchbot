...
%        trash_bin_FaceNormals;
    ...
    methods
        function obj = CollisionCube(x, y, z)
            ...
            axis equal
            vertices = get(CollisionCube_h,'Vertices');
            obj.collisionCube_Vertices = vertices; 
            
            %tablepose = ([-1.75 0 0]);
            collisionCube_pose = ([x, y, z]);
            transformedVertices = [vertices,ones(size(vertices,1),1)] * transl(collisionCube_pose)'* trotx(pi/2)';
            set(CollisionCube_h,'Vertices',transformedVertices(:,1:3));
